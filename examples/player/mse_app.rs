...
/// Simple example of using the gstreamer mse plugin library to run the equivalent of the mse demo
...
use servo_media::player::video::{self, VideoFrameRenderer};
...
impl MseApp {
    pub fn new(path: &Path) -> Result<MseApp, Error> {
        // media file
        ...
        debug_flags.set(DebugFlags::PROFILER_DBG, true);
        ...
        let player = servo_media.create_player(
            ...
            player::StreamType::MSE,
            player_event_sender,
            renderer,
            None,
            Box::new(PlayerContextGlutin::new(true, &windowed_context)),
        );
        ...
        player
            .lock()
            .unwrap()
            .attach_media_source_extension(&media_source)
            .map_err(|error| MiscError(format!("Failed to attach media_source: {error:?}")))?;
        
        println!("attaching extension worked");
...
pub fn mse_main_loop(mut app: MseApp) -> Result<glutin::WindowedContext<glutin::PossiblyCurrent>, Error> {
    ...
    let source = &mut app.media_source;
    ...
    // source mime codec for frag bunny taken from: https://github.com/nickdesaulniers/netfix/blob/gh-pages/demo/bufferAll.html#L14
    let full_source_buffer = source.add_source_buffer("video/mp4; codecs=\"avc1.42E01E, mp4a.40.2\"").expect("Failed to create source buffer");

    let mut file_data = Vec::new();
    file.read_to_end(&mut file_data).expect("Couldn't read file to end");
    let file_buffer = gst::Buffer::from_slice(file_data);

    full_source_buffer.append_buffer(file_buffer).expect("Failed to append file buffer to source buffer");
